 ...
 *
 */
function chainPromises(x, y) {
  let z = Promise.resolve();
  const r = [];
  x.forEach((p) => {
    z = z.then(p.then((res) => r.push(res)).catch((err) => err));
  });
  return z.then(() => r.reduce(y));
}
